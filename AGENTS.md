...
- Write `public` on each member. Do not use the `public extension` pattern
...
- Always use `KabanColor`. Never use raw `Color` or `Color(.system*)` in library code
...
- Apply via `View.kabanTextStyle(_:color:)`. The `color` parameter has no default value and must be explicit
